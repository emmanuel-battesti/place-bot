...
from place_bot.simulation.utils.utils import deg2rad, normalize_angle
from place_bot.simulation.utils.utils_noise import AutoregressiveModelNoise, GaussianNoise


class RobotGPS(Sensor):
    """
    The RobotGPS class is a subclass of Sensor that represents a GPS
    sensor for a robot. It returns the position of the robot as a numpy
    array, with a noise that follows an autoregressive model of order 1.
    """

    def __init__(self, **kwargs):
        """
        Initialize the RobotGPS sensor.
...
class RobotCompass(Sensor):
    """
    RobotCompass sensor returns the orientation of the robot as a float.
    """

    def __init__(self, **kwargs):
        """
        Initialize the RobotCompass sensor.
...
class RobotOdometer(Sensor):
    ...
    RobotOdometer sensor returns a numpy array containing:
      ...
      - alpha: the relative angle of the current position seen from the
        previous reference frame of the robot
      - theta: the variation of orientation (or rotation) of the robot during
        the last step in the reference frame
    """

    def __init__(self, **kwargs):
        """
        Initialize the RobotOdometer sensor.
