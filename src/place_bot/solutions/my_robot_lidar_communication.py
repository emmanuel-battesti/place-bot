...
class MyRobotLidarCommunication(RobotAbstract):
    def __init__(self,
    ...
    def define_message_for_all(self):
        """
        Define the message, the robot will send to and receive from other surrounding robots.
        """
    ...
    def control(self):
        """
        In this example, we only use the lidar sensor and the communication to move the robot
        The idea is to make the robots move like a school of fish.
        The lidar will help avoid running into walls.
        The communication will allow to know the position of the robots in the vicinity, to then correct its own
    ...
    def process_lidar_sensor(self, the_lidar_sensor):
        ...
        # The robot will turn toward the zone with the more space ahead
        if size != 0:
            if far_angle > 0:
        ...
        # If near a wall then 'collision' is True and the robot tries to turn its back to the wall
        collision = False
    ...
    def process_communication_sensor(self):
        ...
        if self.communicator:
            ...
            nearest_robot_coordinate1 = (
                self.measured_gps_position(), self.measured_compass_angle())
            nearest_robot_coordinate2 = deepcopy(nearest_robot_coordinate1)
            (nearest_position1, nearest_angle1) = nearest_robot_coordinate1
            (nearest_position2, nearest_angle2) = nearest_robot_coordinate2

            min_dist1 = 10000
            min_dist2 = 10000
            diff_angle = 0

            # Search the two nearest robots around
            for msg in received_messages:
                ...
                # if another robot is near
                if distance < min_dist1:
                    min_dist2 = min_dist1
                    min_dist1 = distance
                    nearest_robot_coordinate2 = nearest_robot_coordinate1
                    nearest_robot_coordinate1 = coordinate
                    found_robot = True
                elif distance < min_dist2 and distance != min_dist1:
                    min_dist2 = distance
                    nearest_robot_coordinate2 = coordinate

            if not found_robot:
                return found_robot, command_comm

            # If we found at least 2 robots
            if found_robot and len(received_messages) >= 2:
                (nearest_position1, nearest_angle1) = nearest_robot_coordinate1
                (nearest_position2, nearest_angle2) = nearest_robot_coordinate2
            ...
            # If we found only 1 robot
            elif found_robot and len(received_messages) == 1:
                (nearest_position1, nearest_angle1) = nearest_robot_coordinate1
                diff_angle1 = normalize_angle(
                    nearest_angle1 - self.measured_compass_angle())
            ...
            # The robot will slide in the direction of heading
            longi1 = intensity1 * math.cos(heading1)
            lat1 = intensity1 * math.sin(heading1)

            # If we found only 1 robot
            if found_robot and len(received_messages) == 1:
                command_comm["forward"] = longi1
                command_comm["lateral"] = lat1

            # If we found at least 2 robots
            elif found_robot and len(received_messages) >= 2:
